...
async function spinSlot() {
  ...
  // за другом: первый, затем через ~2.5с второй, затем ещё через ~2.5с третий.
  // Пауза одна и та же независимо от выигрыша — не «то короче, то длиннее».
  const START = 1.6, GAP = 2.5;
  ...
  const BRAKE = 0.8;
  // Заглушка едет непрерывно (CSS-анимация) — момент, когда мы её снимаем,
  // может прийтись на ЛЮБУЮ точку цикла, не обязательно на границу ячейки.
  // Лента при этом визуально «доезжает» до ближайшей ровной позиции рывком —
  // именно это и дёргалось в момент старта торможения. Округляем паузу до
  // ближайшей границы ячейки коастинга (COAST_CYCLE/3), чтобы снимать заглушку
  // ровно тогда, когда она и так стоит на целой ячейке — без довыравнивания.
  const CELL_TIME = COAST_CYCLE / 3 * 1000;
  [0, 1, 2].forEach(c => {
    const s = $("strip" + c); if (!s) return;
    const raw = Math.max(0, (durs[c] - BRAKE) * 1000);
    const wait = Math.round(raw / CELL_TIME) * CELL_TIME;
