...
def _ensure_category_columns():
    ...
    if "has_flavors" not in cols:
        ...
        cur.execute(_q("UPDATE categories SET has_flavors = 1 WHERE code IN (%s, %s)"),
                    ("disposable", "liquid"))
    # Как называется то, по чему считается остаток. У одноразок и жидкостей это
    # вкус, у испарителей — сопротивление, у подов — цвет. Механизм один и тот
    # же (метка + остаток), а слово разное: «вкус 0.6 Ом» — это дичь, и продавец
    # такую карточку заводить не станет.
...
# Слово для варианта у стартовых категорий. Разбиралось по живым магазинам:
# у испарителей позиции отличаются сопротивлением и упаковкой
# («0,17 Ом, упак. 3 шт»), у подов — цветом (одна модель в шести расцветках),
# у одноразок, жидкостей и паучей — вкусом.
ВАРИАНТЫ_ПО_УМОЛЧАНИЮ = {
    "disposable": "Вкус", "liquid": "Вкус",
    "coils": "Сопротивление", "podsystem": "Цвет", "devices": "Цвет",
    "accessories": "Вариант",
}

# (код, название, значок, порядок, есть ли варианты).
