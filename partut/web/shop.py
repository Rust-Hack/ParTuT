...
def api_docs():
    ...
    Именно всем: документы, которые видно только вошедшему покупателю, не
    выполняют своей задачи. Кэш длинный — тексты меняются раз в год, а читают
    их с экрана оформления заказа, где каждая лишняя задержка на счету.
    """
    готовое = cache.get("docs")
    if готовое is None:
        документы = db.documents()
        готовое = cache.put("docs", {
            "offer": документы["offer"],
            "privacy": документы["privacy"],
            "version": документы["version"],
        }, 3600)
    return jsonify(готовое)
