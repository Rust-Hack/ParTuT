...
def документы_готовы():
    """Заменил ли владелец черновик оферты и политики своим текстом.

    Пока не заменил, покупателю их не показываем вовсе. Болванка с местами вида
    [УНП] выглядит как настоящий документ и вводит в заблуждение сильнее, чем
    честное отсутствие: человек решит, что условия есть и он их принял.

    Смотрим на оба документа: один готовый, второй черновой — это не готово.
    Кэш общий с прочими настройками магазина, лишнего похода в базу нет.
    """
    from partut.db import shop as db_shop
    готово = cache.get("docs_ready")
    if готово is None:
        готово = cache.put("docs_ready",
                           bool(db.get_setting(db_shop.КЛЮЧ_ОФЕРТЫ)
                                and db.get_setting(db_shop.КЛЮЧ_ПОЛИТИКИ)), 300)
    return готово
