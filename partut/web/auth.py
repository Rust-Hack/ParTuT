...
_OWNER_ONLY = (
    ...
    "/api/admin/staff/",
    # /api/admin/log НЕ здесь: продавцу открыт (маршрут сам сужает до его
    # собственных действий), закрыт только владельцу — весь журнал магазина.
...
def _foreign():
    # Формулировка общая: этой же проверкой закрываются заказы и склад, а не
    # только товары, и «это товар другой точки» на чужом заказе сбивало с толку.
    return jsonify({"ok": False, "error": "other_city",
                    "message": "Это другая точка — её ведёт другой продавец."}), 403
