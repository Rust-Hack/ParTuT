...
Их используют И бот (partut/bot/handlers.py), И веб-сервер Mini App (server.py),
...
# ------------------------------------------------------------------
# Раньше здесь стоял вшитый в код номер. Работало это ровно до вопроса «а как
# его убрать»: ответа не было — только правка исходника и выкатка. Номер
# владельца, лежащий в открытом репозитории, — плохая вещь сама по себе, но
# хуже то, что доступ, который нельзя отозвать, перестаёт быть доступом и
# становится свойством программы.
DEV_IDS = _ids_from_env("DEV_IDS")
...
# Последний ключ от магазина.
#
# SUPER_ADMIN_IDS задают в панели хостинга, и заполнить её забывают: она не
# нужна, чтобы бот заработал, — в отличие от ADMIN_IDS, без которой владелец не
# получит даже админ-меню. Раньше эту дыру затыкал вшитый номер, и вместе с ним
# ушла бы страховка: магазин поднялся бы вообще без хозяина — товары есть,
# продавцы есть, а изменить нельзя ничего и никому.
#
# Поэтому: не назвали владельца явно — им становится тот, кого назвали первым.
if not SUPER_ADMIN_IDS and ADMIN_IDS:
    SUPER_ADMIN_IDS = set(ADMIN_IDS)
    print("SUPER_ADMIN_IDS не задан — владельцем считается ADMIN_IDS: "
          f"{sorted(SUPER_ADMIN_IDS)}. Задайте SUPER_ADMIN_IDS явно.", flush=True)

if not SUPER_ADMIN_IDS:
    raise SystemExit(
        "Не задан ни один владелец магазина — управлять им будет некому.\n"
        "Узнайте свой Telegram-id командой /myid и впишите его:\n"
        "  локально — в файл .env рядом с кодом (SUPER_ADMIN_IDS=...);\n"
        "  на Render — Settings → Environment → переменная SUPER_ADMIN_IDS.\n"
        "После правки переменных нужен перезапуск: они читаются только при старте.")
...
# Тоже переменной, а не номером в коде — по той же причине, что и владелец:
# менеджер меняется, и правка исходника ради этого никуда не годится.
#
# Пусто — вопросы идут владельцу. Молча ронять их нельзя: человек нажал
# «Написать в поддержку» и уверен, что его прочитали.
SUPPORT_IDS = _ids_from_env("SUPPORT_IDS") or set(SUPER_ADMIN_IDS)
...
# Кто умеет прочитать продавцов из базы. Ставит его тот, кто базу и поднял.
#
# Раньше config импортировал db напрямую — и стрелка смотрела не туда: config
# лежит в основании, его импортируют все, и получалось, что «прочитать
# настройки» означало «поднять базу». Теперь основание ничего ни о ком не
# знает, а нужную ему функцию ему приносят.
_читатель_продавцов = None


def set_staff_source(fn):
    """Зарегистрировать источник прав. Зовётся из db при его загрузке."""
    global _читатель_продавцов
    _читатель_продавцов = fn
...
CONFIRM_MINUTES = 15
# Сколько ждём чек по заказу картой, прежде чем отменить его и вернуть товар.
# Живёт здесь, а не в боте: покупателю об этом сроке говорят на экране оплаты,
# и два разных числа в двух местах разъехались бы на первой же правке.
CANCEL_UNPAID_HOURS = 24


# --- Режим разработки ---
# Позволяет открыть приложение в обычном браузере, без Телеграма: подставляется
# первый админ. На боевой базе выключается принудительно — иначе достаточно было
# бы задать переменную, чтобы войти владельцем без всякой проверки подписи.
def dev_mode_allowed(флаг, database_url):
    """Можно ли пускать без Телеграма. Правило вынесено отдельной функцией,
    чтобы его можно было проверить, не перезапуская приложение целиком.

    Единственное «нет», которое здесь важно: боевая база. DEV_MODE подставляет
    владельца любому, кто открыл страницу, — на боевой базе это была бы админка
    без пароля для всего интернета. Поэтому запрет жёсткий, а не «по умолчанию
    выключено».
    """
    return флаг == "1" and not bool((database_url or "").strip())


_IS_PRODUCTION = bool(os.environ.get("DATABASE_URL", "").strip())
DEV_MODE = dev_mode_allowed(os.environ.get("DEV_MODE"), os.environ.get("DATABASE_URL"))
