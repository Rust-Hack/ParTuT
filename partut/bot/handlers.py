...
# --- Уборка за обработчиками -------------------------------------------
# Телебот исполняет каждое обновление в одном из своих рабочих потоков и
# переиспользует их. Значит соединение, брошенное упавшим обработчиком, висит
# там до конца жизни процесса — та же утечка, что и в вебе (см.
# db.вернуть_забытые), только заметить её ещё труднее: у бота нет ни запроса,
# ни ответа, по которым видно, что что-то пошло не так.
#
# Точка одна на все обработчики. Расставлять уборку по девяти командам значит
# забыть про десятую — ровно так же, как забыли бы про журнал действий.
assert hasattr(bot, "_exec_task"), \
    ("у телебота больше нет _exec_task — уборка за обработчиками потеряна. "
     "Найдите новую точку, через которую он исполняет обновления, "
     "и перевесьте на неё db.вернуть_забытые.")

_исполнить_задачу = bot._exec_task


def _задача_с_уборкой(задача, *args, **kwargs):
    def обёрнутая(*a, **k):
        try:
            return задача(*a, **k)
        finally:
            ...
            if забыто:
                print(f"[база] бот {getattr(задача, '__name__', задача)}: "
                      f"подобрано брошенных соединений: {забыто}", flush=True)
    return _исполнить_задачу(обёрнутая, *args, **kwargs)


bot._exec_task = _задача_с_уборкой
...
def handle_admin_callback(call, chat_id, user_id, data):
    ...
    if data == "adm:add":
        ...
        bot.answer_callback_query(call.id)
        # Путь завоза переехал: раньше он начинался кнопкой у модели в
        # «Ассортименте», теперь живёт там, где работает продавец. Текст,
        # который называет несуществующую кнопку, хуже отсутствия текста:
        # человек ищет её и решает, что сломалось приложение.
        bot.send_message(chat_id,
                         "Товары заводятся в приложении:\n\n"
                         "1. 📚 Ассортимент — описать модель: бренд, характеристики, "
                         "варианты (вкус, сопротивление или цвет — смотря какая категория), фото\n"
                         "2. 🏬 Цены и остатки → «➕ Завезти на точку» — цена, закупка и остаток\n\n"
                         "Так описание модели одно на все точки и правится в одном месте, "
                         "а цена и остаток — свои у каждой точки.")
        return
...
def _send_backup(chat_ids, note=""):
    """Отправляет копию в Телеграм И во внешнее хранилище.

    Два адресата намеренно. Телеграм удобен — копия под рукой в переписке. Но
    он же и тот аккаунт, которым владелец открывает магазин и в котором лежит
    токен бота: потеря доступа отняла бы разом и управление, и все копии.

    Возвращает текст ошибки, если копия не ушла НИКУДА. Если хоть один адресат
    принял — это не провал, но о втором всё равно скажем в подписи: владелец
    должен видеть состояние второй копии каждый день, а не выяснять его в тот
    единственный день, когда она понадобится.
    """
    try:
    ...
    беда_хранилища = offsite.отправить(blob, name) if offsite.настроено() else None
    if беда_хранилища:
        # Подписи под файлом мало: её листают глазами. Молчаливо не работающая
        # вторая копия — ровно та беда, ради которой её и заводили.
        errors.report(bot, "вторая копия базы", RuntimeError(беда_хранилища))
    if offsite.настроено():
        вторая = (f"✅ вторая копия в хранилище ({offsite.хранилище()})" if not беда_хранилища
                  else f"⚠️ вторая копия НЕ ушла: {беда_хранилища}")
    else:
        вторая = ("ℹ️ вторая копия не настроена — сейчас всё лежит в одном "
                  "аккаунте Телеграма (BACKUP_URL)")

    size = f"{len(blob) / 1024:.0f} КБ"
    caption = ((note or "🗄 Резервная копия базы")
    ...
    if sent:
        return None
    # Телеграм не принял. Если хранилище приняло — данные целы, и поднимать
    # тревогу «копии нет» было бы враньём.
    if offsite.настроено() and not беда_хранилища:
        return None
...
def _close_raffle():
    """Подвести итоги розыгрыша, если срок вышел.

    Раньше это делал тот покупатель, который первым открыл вкладку после срока.
    Если не открыл никто — розыгрыш висел законченным, а призы никому не
    достались. Дело магазина не должно зависеть от того, заглянул ли кто-то."""
    ...
    for name, step in _BACKGROUND_STEPS:
        try:
            try:
                ...
                # Цикл нарочно редкий — раз в 15 минут, чтобы дать Neon
                # заснуть между тиками (см. docstring _reminder_loop) — и это
                # ровно то время, за которое Neon рвёт простаивающие
                # соединения. Первый запрос после паузы иногда достаёт из
                # пула уже мёртвое соединение: psycopg2 узнаёт об этом только
                # на самом запросе, не раньше. Не баг — обычное дело для
                # засыпающей базы. Одна попытка с чистым соединением обычно
                # чинит его сама же, без письма владельцу о том, что уже
                # разрешилось само.
                stale = db.USE_PG and isinstance(e, db.psycopg2.OperationalError)
                if not stale:
                    raise
                db.вернуть_забытые()
                step()
            ...
            # Этот цикл живёт вечно в своём потоке: упавшее дело унесло бы с
            # собой соединение навсегда. Раз в минуту, семь дел — пул кончился
            # бы за три минуты.
            забыто = db.вернуть_забытые()
            if забыто:
                print(f"[база] фоновые дела ({name}): "
                      f"подобрано брошенных соединений: {забыто}", flush=True)


_TICK_SECONDS = int(os.environ.get("BG_TICK_SECONDS", "900"))  # 15 минут по умолчанию


def _reminder_loop():
    """Прокручивает фоновые дела магазина.

    Раз в 15 минут, не раз в минуту (29 августа 2026, решение владельца).
    Каждый тик — это подключение к Neon, а бесплатный тариф считает квоту по
    времени, пока база АКТИВНА: Neon засыпает после ~5 минут без запросов,
    и минутный цикл не давал ей поспать вообще никогда — база числилась
    работающей круглые сутки, даже ночью без единого покупателя. Это и
