...
def run_address_migration():
    """Перенос 0006: адрес, вписанный в сам способ, становится точкой города.

    Так адрес перестаёт жить в двух местах. Проверяем и обратное: заглушку из
    стартового засева переносить нельзя — точка с текстом «Уточните адрес…»
    отправила бы покупателя по несуществующему адресу.
    """
    c = Checker("Перенос: адрес самовывоза стал точкой")
    _clean()

    db.add_delivery_method("Минск", "Самовывоз", False, "", "ул. Кирова 3", 0, True, 0)
    db.add_delivery_method("Минск", "Самовывоз-2", False, "", "ул. Кирова 3", 0, True, 1)
    db.add_delivery_method("Туров", "Забрать", False, "", db.ЗАГЛУШКА_САМОВЫВОЗА, 0, True, 0)
    db.add_delivery_method("Туров", "Курьер", True, "Адрес", "ул. Ленина 1", 0, True, 1)

    заведено = db._pickup_addresses_to_points()
    адреса = [p["address"] for p in db.get_pickup_points("Минск")]
    c("адрес из способа стал точкой", адреса == ["ул. Кирова 3"])
    c("одинаковый адрес не задваивается", заведено == 1)
    c("заглушка не переносится", db.get_pickup_points("Туров") == [])

    # Второй прогон ничего не добавляет: перенос обязан быть безобидным при
    # повторе — летопись снимает заявку, если шаг упал, и он пойдёт снова.
    c("повторный прогон ничего не создаёт", db._pickup_addresses_to_points() == 0)
    c("точка так и одна", len(db.get_pickup_points("Минск")) == 1)

    # Курьерский способ не самовывоз: его строку трогать нельзя.
    c("адрес курьерского способа не переносится",
      "ул. Ленина 1" not in [p["address"] for p in db.get_pickup_points("Туров")])
    _clean()
    return c.fails
