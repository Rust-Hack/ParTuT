...
def run_настройки():
    ...
    # Список превращается в "" — а пустые реквизиты сервер отказывается
    # сохранять: без них покупателю платить некуда, и «Сохранено ✅» на
    # пустом поле сказало бы, что так и задумано. Прежние реквизиты целы.
    до = client.post("/api/admin/settings", json={"initData": "x"}).get_json()["settings"]["payment_info"]
    c("список вместо строки не роняет и не стирает", сохранить_и_прочитать("payment_info", ["a"]) == до)
    код, о = _post("/api/admin/settings/update", payment_info="")
    c("пустая строка отклонена, а не сохранена",
      о.get("failed", {}).get("payment_info") and
      client.post("/api/admin/settings", json={"initData": "x"}).get_json()["settings"]["payment_info"] == до)
